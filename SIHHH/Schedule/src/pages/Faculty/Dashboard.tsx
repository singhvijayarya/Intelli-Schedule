  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6">
      ...
      <div className="text-center mb-12 relative">
        ...
        >
          Welcome back, <span className="text-amber-400 font-semibold">{user?.name || 'Admin'}</span>
        </motion.p>
      </div>
        ...
        {dashboardCards.map((card) => (
          ...
          >
            <div className="relative">
              <div 
                className="absolute -inset-0.5 rounded-2xl blur opacity-0 group-hover:opacity-100 transition duration-300"
                style={{ background: `linear-gradient(45deg, ${card.glow}, transparent)` }}
              />
              <div className="relative rounded-2xl p-6 bg-slate-800/40 backdrop-blur-xl border border-slate-700/50 group-hover:border-slate-600/70 transition-all duration-300">
                <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${card.color} flex items-center justify-center text-2xl mb-4 transform group-hover:scale-110 transition-transform duration-300 shadow-lg`}>
                  {card.icon}
                </div>
                ...
                {/* Simplified shimmer effect without complex transforms */}
                <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-transparent via-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
              </div>
            </div>
          </motion.div>
        ))}
      ...
      >
        <p className="text-slate-500 text-sm">
          Intelli-Schedule Admin Portal • SIH25091
        </p>
      </motion.div>
    </div>
  );
